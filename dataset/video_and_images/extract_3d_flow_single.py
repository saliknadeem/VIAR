...
def main():
  ...
  if not os.path.isdir(args.output_dir):
    ...
    print("Created output dir: "+args.output_dir)
  ind=0
  length=1

  if args.num_worker < 1:
    extract_flow(args, ind, args.rgb_dir,args.depth_dir, length=1)
  else:
    from joblib import Parallel, delayed
    Parallel(n_jobs=args.num_worker)(
      delayed(extract_flow)(args, ind, args.rgb_dir,args.depth_dir, length)
      )
...
def extract_flow(args, ind, rgbs_dir,depths_dir, length):
  ...
  try:
    ...
    if result == 0:
      print('Extracted 3D flow ...took {}'.format( timeSince(starttime)) )
    else:
      print('Error: {}/{}. Failed to extract 3D flow from {}...took {}'.format(
        ind+1, length, timeSince(starttime)) )
